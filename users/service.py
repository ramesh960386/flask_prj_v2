...
def create_task(request, input_data):
    create_validation_schema = TaskSchema()
    errors = create_validation_schema.validate(input_data)
    ...
    return generate_response(
        data=task_schema.dump(task_data), message="Task Created", status=HTTP_201_CREATED
    )

def get_task_list(request):
    task_data = Task.find_all()
    if task_data is None:
        raise CustomException("Task not found", 404)
    return generate_response(
        data=tasks_schema.dump(task_data), message="Task Listed", status=HTTP_200_OK
    )


def get_task(request, id):
    task_data = Task.find_by_id(id)
    if task_data is None:
        raise CustomException("Task not found", 404)
    return generate_response(
        data=task_schema.dump(task_data), message="Task details", status=HTTP_200_OK
    )

def update_task(request, input_data, id):
    create_validation_schema = TaskSchema()
    errors = create_validation_schema.validate(input_data)
    ...
    if task_data is None:
        raise CustomException("Task not found", 404)
    task_data.name = input_data.get('name')
    task_data.save_to_db()
    # db.session.add(task_data)
    # db.session.commit()
    return generate_response(
        data=input_data, message="Task Updated", status=HTTP_200_OK
    )
...
def delete_task(request, id):
    ...
    return generate_response(
        data=task_schema.dump(task_data), message="Task was deleted", status=HTTP_200_OK
    )
